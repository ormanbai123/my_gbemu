...
void draw_scanline() {
	...
	if (sprites_enabled()) {
		...
		uint16_t sprite_base = 0x8000;
		uint16_t sprite_offset;

		uint8_t palette, bg_priority;

		for (auto _ = 0; _ < g_ppu->spriteBuff.size; ++_) {
			...
			sprite_offset = ((int16_t)g_ppu->ly - obj.posY) & 7;
			if (yflip == true)
				sprite_offset = ~sprite_offset;
			sprite_offset = (sprite_offset & 0b111) << 1;

			tile_id = obj.tileIndx;
			...
			for (auto col = 0; col < 8; ++col) {
				...
				if (color_id > 0) {
					if (bg_priority == 0 || scanline_color_ids[sprite_x] == 0) {
						g_ppu->screenBuffer[160 * g_ppu->ly + sprite_x] = RAY_COLORS[color_id];
					}
				}
			}
